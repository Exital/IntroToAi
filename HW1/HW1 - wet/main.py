...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    expand_list = []
    weight_list = []
    weights = np.linspace(low_heuristic_weight, high_heuristic_weight, num=n)
    for w in weights:
        alg = AStar(heuristic_type, w, max_nr_states_to_expand)
        res = alg.solve_problem(problem)
        if res.is_solution_found:
            weight_list.append(w)
            expand_list.append(res.nr_expanded_states)
...
def toy_map_problem_experiments():
    ...
    astary = AStar(NullHeuristic)
    res = astary.solve_problem(toy_map_problem)
    print(res)

    # Ex.13
    # TODO: create an instance of `AStar` with the `AirDistHeuristic`,
    #       solve the same `toy_map_problem` with it and print the results (as before).
    astar_with_air = AStar(AirDistHeuristic)
    res = astar_with_air.solve_problem(toy_map_problem)
    print(res)
...
def mda_problem_with_astar_experiments():
    ...
    astarmax = AStar(MDAMaxAirDistHeuristic)
    res = astarmax.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
    ...
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    astarsum = AStar(MDASumAirDistHeuristic)
    res = astarsum.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)

    # Ex.28
    # TODO: create an instance of `AStar` with the `MDAMSTAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    astarmst = AStar(MDAMSTAirDistHeuristic)
    res = astarmst.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)
...
def multiple_objectives_mda_problem_experiments():
    ...
    astartravel = AStar(MDATestsTravelDistToNearestLabHeuristic)
    res = astartravel.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
...
def mda_problem_anytime_astar_experiments():
    ...
    astar = AnytimeAStar(MDAMSTAirDistHeuristic, 1000)
